...
class SubNameBrute:
    ...
    def _load_sub_names(self):
        if self.options.full_scan and self.options.file == 'subnames.txt':
            _file = pwd+'/dict/subnames_full.txt'
        else:
            if os.path.exists(self.options.file):
                _file = self.options.file
            elif os.path.exists(pwd+'/dict/%s' % self.options.file):
                _file = pwd+'/dict/%s' % self.options.file
            else:
    ...
    def _scan(self, j):
        ...
        global sub_domain
        global break_flag
        while not self.queue.empty() and break_flag == 0:
            try:
            ...
            try:
                ...
                if answers:
                    ...
                    if self.found_count.value > 2000:
                        break_flag = 1
                    ...
                    # sub_domain.append(cur_sub_domain)
                    self.outfile.flush()
                    try:
...
if __name__ == '__main__':
    ...
    dns_servers = load_dns_servers(pwd)
    next_subs = load_next_sub(options,pwd)
