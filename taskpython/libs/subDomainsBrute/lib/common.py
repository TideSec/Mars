...
def load_dns_servers(pwd):
    print_msg('[+] Validate DNS servers', line_feed=True)
    dns_servers = []
    pool = Pool(10)
    for server in open(pwd+'/dict/dns_servers.txt').readlines():
        server = server.strip()
        if server:
...
def load_next_sub(options,pwd):
    next_subs = []
    _set = set()
    _file = pwd+'/dict/next_sub_full.txt' if options.full_scan else pwd+'/dict/next_sub.txt'
